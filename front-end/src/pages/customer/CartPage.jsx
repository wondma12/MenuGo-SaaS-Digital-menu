...
const CartPage = () => {
  ...
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === 'menugo_cart') {
        try {
          ...
          setCartCount(raw ? JSON.parse(raw).reduce((s, i) => s + i.quantity, 0) : 0);
        } catch (err) {
          setCartCount(0);
        }
      }
    ...
    const onCustom = (e) => {
      ...
      if (Array.isArray(updated)) setCartCount(updated.reduce((s, i) => s + i.quantity, 0));
      else {
        try {
          const raw = e.newValue;
          setCartCount(raw ? JSON.parse(raw).reduce((s, i) => s + i.quantity, 0) : 0);
  ...
  const updateQuantity = (id, quantity) => {
    if (quantity <= 0) {
      ...
      setCartItems(next);
      try { sessionStorage.setItem('menugo_cart', JSON.stringify(next)); } catch (e) {}
    } else {
  ...
  const handlePlaceOrder = () => {
    ...
    const itemsSummary = cartItems.map(it => `${it.name} x ${it.quantity} — $${(it.price * it.quantity).toFixed(2)}`).join('\n');
    ...
    const next = [];
    setCartItems(next);
    try { sessionStorage.removeItem('menugo_cart'); } catch (e) {}
    navigate('/');
  };
  ...
  React.useEffect(() => {
    if (!location.state?.cart) {
      ...
      try { sessionStorage.setItem('menugo_cart', JSON.stringify(location.state.cart)); } catch (e) {}
    }
  ...
  const Header = () => (
    <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 py-4 md:px-12 bg-white border-b border-neutral-200">
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        <Link to="/cart" className="relative flex items-center gap-2 text-neutral-600 hover:text-neutral-900">
          <span className="material-symbols-outlined"></span>
          <span className="font-label-caps text-label-caps">Orders</span>
          {cartCount > 0 && (
            <span className="absolute -top-2 -right-3 bg-primary text-on-primary text-[11px] font-bold px-2 py-0.5 rounded-full">{cartCount}</span>
          )}
        </Link>
      </div>
    </header>
  ...
  const BottomNav = () => (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-3 pb-safe bg-white/95 backdrop-blur-md border-t border-neutral-200 shadow-soft md:hidden">
      <Link to="/customer" className="flex flex-col items-center justify-center text-neutral-400">
        <span className="material-symbols-outlined"></span>
      ...
      <Link to="/cart" className="flex flex-col items-center justify-center px-3 py-1 rounded-md bg-neutral-100 text-neutral-900 relative">
        <span className="material-symbols-outlined"></span>
        <span className="font-['Inter'] text-[10px] font-bold uppercase tracking-widest mt-1">Orders</span>
        {cartCount > 0 && <span className="absolute -top-1 right-6 bg-primary text-on-primary text-[11px] font-bold px-2 py-0.5 rounded-full">{cartCount}</span>}
      </Link>
    </nav>
  );
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="pt-32 pb-24 max-w-container-max mx-auto px-6 text-center">
          ...
          <p className="text-secondary mt-2">Start adding delicious items from the menu.</p>
          <Link to="/customer" className="mt-6 inline-block bg-black text-white px-6 py-3 rounded-lg">Browse Menu</Link>
        </main>
        <BottomNav />
        <Footer />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-7 space-y-6">
            <header>
              ...
              <p className="font-body-md text-on-secondary-container mt-xs">Review your selection before placing order.</p>
            </header>
          ...
          <aside className="lg:col-span-5 space-y-6 lg:sticky lg:top-20">
            <OrderSummary subtotal={getSubtotal()} onPlaceOrder={handlePlaceOrder} />
          </aside>
        </div>
      </main>
      <BottomNav />
