...
const MenuPage = () => {
  ...
  React.useEffect(() => {
    ...
    menuService.getCategories().then((res) => {
      if (!mounted) return;
      if (res.success && Array.isArray(res.data)) setCategories(['ALL', ...res.data.map(c => c.toUpperCase())]);
    });
  ...
  const filteredItems =
    activeCategory === 'ALL'
      ? menuItems
      : menuItems.filter((item) => item.category?.toUpperCase() === activeCategory);
  ...
  const Header = () => (
    <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 py-4 md:px-12 bg-white border-b border-neutral-200">
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        <Link to="/cart" state={{ cart }} className="relative flex items-center gap-2 text-neutral-600 hover:text-neutral-900">
          <span className="material-symbols-outlined"></span>
  ...
  const BottomNav = () => (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-3 pb-safe bg-white/95 backdrop-blur-md border-t border-neutral-200 shadow-soft md:hidden">
      ...
      <Link to="/cart" className="flex flex-col items-center justify-center text-neutral-400 relative">
        <span className="material-symbols-outlined"></span>
        <span className="font-['Inter'] text-[10px] font-bold uppercase tracking-widest mt-1">Orders</span>
        {cartCount > 0 && <span className="absolute -top-1 right-8 bg-primary text-on-primary text-[11px] font-bold px-2 py-0.5 rounded-full">{cartCount}</span>}
      </Link>
    </nav>
  );
      ...
      <div className="max-w-container-max mx-auto px-6 py-12 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="text-center md:text-left">
          ...
          <p className="font-['Inter'] text-xs font-light text-neutral-500 mt-1">
            © {new Date().getFullYear()} LUMIÈRE DINING. POWERED BY MENU-SAAS.
          </p>
        </div>
