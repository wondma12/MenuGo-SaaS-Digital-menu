    ...
    <Card title="Theme Customizer">
      <div className="space-y-5">
        ...
        <div>
          ...
          >
            <p className="text-gray-700">Preview: The quick brown fox jumps over the lazy dog</p>
          </div>
        </div>
            ...
            {buttonStyles.map((style) => (
              <button
                ...
                className={`px-4 py-2 bg-black text-white text-sm transition-all ${
                  ...
                  style.value === "rounded-lg" ? "rounded-lg" :
                  style.value === "rounded-full" ? "rounded-full" : ""
        ...
        <div>
          ...
          <button
            ...
            className={`flex items-center justify-between w-full p-3 rounded-lg transition-colors ${
              theme.darkMode ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-700"
            }`}
          >
            <div className="flex items-center gap-2">
              {theme.darkMode ? <Moon size={18} /> : <Sun size={18} />}
              <span>{theme.darkMode ? "Dark Mode Enabled" : "Light Mode Enabled"}</span>
            </div>
            <div className={`w-10 h-5 rounded-full transition-colors ${theme.darkMode ? "bg-blue-600" : "bg-gray-300"}`}>
              <div className={`w-4 h-4 rounded-full bg-white transition-transform mt-0.5 ${theme.darkMode ? "translate-x-5" : "translate-x-0.5"}`} />
            </div>
          </button>
        </div>
        ...
        <div className="bg-blue-50 p-3 rounded-lg">
          <p className="text-xs text-blue-700">
            💡 Tip: Changes here will affect how your menu appears to customers. Click "Preview" to see the live effect.
          </p>
        </div>
      </div>
    </Card>
